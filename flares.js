...
class LinkingManager {
    ...
    static async sendFlareToLinkedContacts(sessionData, selectedContactIds = null) {
        console.log('[DEBUG] sendFlareToLinkedContacts called', { timestamp: new Date().toISOString(), selectedContactIds });
        ...
        for (const contact of linkedContacts) {
            try {
                console.log(`[DEBUG] Writing to inbox for user: ${contact.userId} at ${new Date().toISOString()}`);
                ...
                console.log(`[DEBUG] Inbox write complete for user: ${contact.userId}, docId: ${docRef.id}`);
        ...
        console.log(`[DEBUG] sendFlareToLinkedContacts complete. Sent flares to ${linkedContacts.length} contacts`);
    }
}
...
class InboxManager {
    ...
    static async renderInboxScreen() {
        console.log('[DEBUG] renderInboxScreen called');
        const inboxList = document.getElementById('inboxList');
        const emptyState = document.getElementById('inboxEmptyState');

        if (!inboxList) {
            console.error('[DEBUG] inboxList element not found');
            return;
        }

        const items = await this.fetchAllInboxItems();
        console.log('[DEBUG] Fetched inbox items:', items.length, items);
        ...
        if (items.length === 0) {
            console.log('[DEBUG] No inbox items, showing empty state');
            if (emptyState) emptyState.style.display = 'block';
            return;
        }

        console.log('[DEBUG] Rendering', items.length, 'inbox items');
...
async function initApp() {
    ...
    // Global debounce to prevent ANY double-sends on iOS (shared across both buttons)
    let isSendingAnything = false;
    ...
    if (sendToLinkedBtn) {
        sendToLinkedBtn.addEventListener('click', async () => {
            if (isSendingAnything) {
                console.log('[DEBUG] Send blocked - already sending');
                return;
            }
            isSendingAnything = true;
            ...
            // Reset after success modal shows
            setTimeout(() => {
                isSendingAnything = false;
            }, 2000);
        });
    ...
    // History button
    // Settings (with debounce to prevent click + touchend double-firing on iOS)
    let settingsOpening = false;
    const openSettings = (e) => {
        e.preventDefault();
        e.stopPropagation();

        // Prevent duplicate calls from click + touchend firing together
    ...
    // iOS Safari sometimes needs touchend as well
    ...
    document.getElementById('enableNotificationsBtn')?.addEventListener('click', async () => {
        ...
        console.log('[DEBUG] User authenticated:', !!AuthManager.currentUser);

        if (!AuthManager.currentUser) {
            alert('Please sign in first to enable push notifications.');
            return;
        }
        ...
        try {
            ...
            if (permission === 'granted') {
                ...
                PushNotificationManager.updateNotificationStatus();
                alert('✅ Notifications enabled! You will now receive Flares from your contacts.');
            } else if (permission === 'denied') {
                PushNotificationManager.updateNotificationStatus();
                alert('❌ Notifications were blocked. To enable:\n\niPhone: Settings → Safari → Websites → Notifications → Allow\n\nThen refresh this page and try again.');
            } else {
                console.log('[DEBUG] Permission was default/dismissed');
                PushNotificationManager.updateNotificationStatus();
                alert('Notification permission was not granted. Please try again.');
            }
        } catch (error) {
            console.error('[DEBUG] Error enabling notifications:', error);
